/*configure_sql_server_backups*/

--Create the DBA_ToolBox
 
USE master;
GO
CREATE DATABASE DBA_ToolBox;
GO
...
USE [DBA_ToolBox] -- Specify the database in which the objects will be created.
...
SET @BackupDirectory     = N'H:\MSSQL\BACKUP' -- Specify the backup root directory.
...
SET @LogToTable          = 'Y'          -- Log commands to a table.
  ...
  IF LEFT(CAST(SERVERPROPERTY('ServerName') AS VARCHAR),3) = 'UAT' OR LEFT(CAST(SERVERPROPERTY('ServerName') AS VARCHAR),3) = 'DEV'
  BEGIN
	SET @CleanupTime = 24
  END
  ELSE
...
BEGIN
  ...
  DECLARE @OperatorName nvarchar(max)
  DECLARE @WhoToNotify nvarchar(max)
 --QM CUSTOM END
  ...
  --QM CUSTOM
  DECLARE @JobName12 nvarchar(max)
  DECLARE @JobName13 nvarchar(max)
  --QM CUSTOM END
  ...
  DECLARE @JobCommand11 nvarchar(max)
    --QM CUSTOM
  DECLARE @JobCommand12 nvarchar(max)
  DECLARE @JobCommand13 nvarchar(max)
   --QM CUSTOM END
  ...
  DECLARE @OutputFile11 nvarchar(max)
    --QM CUSTOM
  DECLARE @OutputFile12 nvarchar(max)
  DECLARE @OutputFile13 nvarchar(max)
 
  ----------------------------------------------------------------------------------------------------
  --// If DEV or UAT server, keep backup for 24 hours. Else 72 hours for production               //--
  ----------------------------------------------------------------------------------------------------
 
  IF LEFT(CAST(SERVERPROPERTY('ServerName') As Varchar),3) = 'UAT' OR LEFT(CAST(SERVERPROPERTY('ServerName') As Varchar),3) = 'DEV'
  BEGIN
	SET @BackupRetention = 24
  END
  ELSE
  BEGIN
	SET @BackupRetention = 72
  END
   --QM CUSTOM END
  ...
  --QM CUSTOM
  SET @WhoToNotify = 'its-dba-team@qmul.ac.uk'
  SET @OperatorName = 'All_Operators'
 
  IF Exists (SELECT loginname FROM master.dbo.syslogins WHERE name = 'dba-sysadmin')
  BEGIN
	SET @JobOwner = 'dba-sysadmin'
  END
  ELSE
  BEGIN
	SET @JobOwner = 'sa'
  ...
  --QM CUSTOM
  SET @JobName12 = 'Daily backup AND maintenance - USER_DATABASES'
  SET @OutputFile12 = @OutputFileDirectory + '\' + 'DailyDatabaseMaintUserDBs_' + @TokenJobID + '_' + @TokenStepID + '_' + @TokenDate + '_' + @TokenTime + '.txt'
  IF LEN(@OutputFile12) > 200 SET @OutputFile12 = @OutputFileDirectory + '\' + @TokenJobID + '_' + @TokenStepID + '_' + @TokenDate + '_' + @TokenTime + '.txt'
  IF LEN(@OutputFile12) > 200 SET @OutputFile12 = NULL
 
  SET @JobName13 = 'Daily backup AND maintenance - SYSTEM_DATABASES'
  SET @OutputFile13 = @OutputFileDirectory + '\' + 'DailyDatabaseMaintSystemDBs_' + @TokenJobID + '_' + @TokenStepID + '_' + @TokenDate + '_' + @TokenTime + '.txt'
  IF LEN(@OutputFile13) > 200 SET @OutputFile13 = @OutputFileDirectory + '\' + @TokenJobID + '_' + @TokenStepID + '_' + @TokenDate + '_' + @TokenTime + '.txt'
  IF LEN(@OutputFile13) > 200 SET @OutputFile13 = NULL
  ...
  IF NOT EXISTS (SELECT [name] FROM msdb.dbo.sysjobs WHERE [name] = @JobName12)
  BEGIN
	EXECUTE msdb.dbo.sp_add_job @job_name = @JobName12, @description = @JobDescription, @notify_level_eventlog = 2, @notify_level_email = 2, @notify_email_operator_name = @OperatorName, @category_name = @JobCategory, @owner_login_name = @JobOwner, @job_id = @JobID OUTPUT
	EXECUTE msdb.dbo.sp_add_jobstep @job_name = @JobName12, @step_name = @JobName03, @subsystem = 'CMDEXEC', @command = @JobCommand03, @output_file_name = @OutputFile12, @step_id=1, @on_success_action=3, @on_success_step_id=0, @on_fail_action=2, @on_fail_step_id=0
	EXECUTE msdb.dbo.sp_add_jobstep @job_name = @JobName12, @step_name = @JobName06, @subsystem = 'CMDEXEC', @command = @JobCommand06, @output_file_name = @OutputFile12, @step_id=2, @on_success_action=3, @on_success_step_id=0, @on_fail_action=2, @on_fail_step_id=0
	EXECUTE msdb.dbo.sp_add_jobstep @job_name = @JobName12, @step_name = @JobName07, @subsystem = 'CMDEXEC', @command = @JobCommand07, @output_file_name = @OutputFile12, @step_id=3, @on_success_action=3, @on_success_step_id=0, @on_fail_action=2, @on_fail_step_id=0
	EXECUTE msdb.dbo.sp_add_jobstep @job_name = @JobName12, @step_name = @JobName11, @subsystem = 'CMDEXEC', @command = @JobCommand11, @output_file_name = @OutputFile12, @step_id=4, @on_success_action=3, @on_success_step_id=0, @on_fail_action=2, @on_fail_step_id=0
	EXECUTE msdb.dbo.sp_add_jobstep @job_name = @JobName12, @step_name = @JobName09, @subsystem = 'CMDEXEC', @command = @JobCommand09, @output_file_name = @OutputFile12, @step_id=5, @on_success_action=3, @on_success_step_id=0, @on_fail_action=2, @on_fail_step_id=0
	EXECUTE msdb.dbo.sp_add_jobstep @job_name = @JobName12, @step_name = @JobName08, @subsystem = 'CMDEXEC', @command = @JobCommand08, @output_file_name = @OutputFile12, @step_id=6, @on_success_action=1, @on_success_step_id=0, @on_fail_action=2, @on_fail_step_id=0
	EXECUTE msdb.dbo.sp_add_jobserver @job_name = @JobName12
 
	SET @ScheduleName = @JobName12 + ' schedule'
 
	EXECUTE msdb.dbo.sp_add_jobschedule @job_name=@JobName12, @name=@ScheduleName, 
		@enabled=1, 
		@freq_type=4, 
		@freq_interval=1, 
		@freq_subday_type=1, 
		...
		@active_start_date=20140324,
		@active_end_date=99991231, 
		@active_start_time=223000, 
		@active_end_time=235959
  END
 
  IF NOT EXISTS (SELECT [name] FROM msdb.dbo.sysjobs WHERE [name] = @JobName13)
  BEGIN
	EXECUTE msdb.dbo.sp_add_job @job_name = @JobName13, @description = @JobDescription, @notify_level_eventlog = 2, @notify_level_email = 2, @notify_email_operator_name = @OperatorName, @category_name = @JobCategory, @owner_login_name = @JobOwner
	EXECUTE msdb.dbo.sp_add_jobstep @job_name = @JobName13, @step_name = @JobName01, @subsystem = 'CMDEXEC', @command = @JobCommand01, @output_file_name = @OutputFile13, @step_id=1, @on_success_action=3, @on_success_step_id=0, @on_fail_action=2, @on_fail_step_id=0
	EXECUTE msdb.dbo.sp_add_jobstep @job_name = @JobName13, @step_name = @JobName05, @subsystem = 'CMDEXEC', @command = @JobCommand05, @output_file_name = @OutputFile13, @step_id=2, @on_success_action=1, @on_success_step_id=0, @on_fail_action=2, @on_fail_step_id=0
	EXECUTE msdb.dbo.sp_add_jobserver @job_name = @JobName13
 
	SET @ScheduleName = @JobName13 + ' schedule'
 
	EXECUTE msdb.dbo.sp_add_jobschedule @job_name=@JobName13, @name=@ScheduleName, 
		@enabled=1, 
		@freq_type=4, 
		@freq_interval=1, 
		@freq_subday_type=1, 
  ...
  -------------------------------------------------------------------------------------------------
  --// Add Schedules to Jobs													               //--
  -------------------------------------------------------------------------------------------------
 
  	EXECUTE msdb.dbo.sp_add_jobschedule @job_name='DatabaseBackup - USER_DATABASES - LOG', @name='DatabaseBackup - USER_DATABASES - LOG Schedule', 
	@enabled=1, 
	@freq_type=4, 
	@freq_interval=1, 
	@freq_subday_type=8, 
	...
	EXECUTE msdb.dbo.sp_add_jobschedule @job_name='IndexOptimize - USER_DATABASES', @name='IndexOptimize - USER_DATABASES Schedule', 
	@enabled=1, 
	@freq_type=8, 
	@freq_interval=64, 
	@freq_subday_type=1, 
	...
	@active_end_time=235959
		...
		EXECUTE msdb.dbo.sp_add_jobschedule @job_name='sp_delete_backuphistory', @name='sp_delete_backuphistory Schedule', 
	@enabled=1, 
	@freq_type=8, 
	@freq_interval=64, 
	@freq_subday_type=1, 
	...
	@active_end_time=235959
		...
		EXECUTE msdb.dbo.sp_add_jobschedule @job_name='sp_purge_jobhistory', @name='sp_purge_jobhistory Schedule', 
	@enabled=1, 
	@freq_type=8, 
	@freq_interval=64, 
	@freq_subday_type=1, 
	...
	@active_end_time=235959
		...
		EXECUTE msdb.dbo.sp_add_jobschedule @job_name='Output File Cleanup', @name='Output File Cleanup Schedule', 
	@enabled=1, 
	@freq_type=8, 
	@freq_interval=64, 
	@freq_subday_type=1, 
	...
	@active_end_time=235959
		...
		EXECUTE msdb.dbo.sp_add_jobschedule @job_name='CommandLog Cleanup', @name='CommandLog Cleanup Schedule', 
	@enabled=1, 
	@freq_type=8, 
	@freq_interval=64, 
	@freq_subday_type=1, 
