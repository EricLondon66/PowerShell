...
#                   resources like CSS, JS, and images are included.  Please modify lines 75-92 to suit your
...
Function WarmUp() {
	...
	switch -wildcard ($caTitle) {
		"*PROD*" {
			https://bir.qmul.ac.uk
			http://bir-web-01:45482/
			http://bir-web-01/
			https://bir.qmul.ac.uk/qmul/BISearch/
			https://bir.qmul.ac.uk/qmul/Custom/
			http://biprodap.bir.qmul.ac.uk/
			http://prodsearch.bir.qmul.ac.uk/
		}
		"*TEST*" {
...
Function IENavigateTo([string] $url, [int] $delayTime = 500) {
	...
	try {
		...
		#Change $pid in the original script to $pidProcess to eliminate the error that appears to make the script hang up
