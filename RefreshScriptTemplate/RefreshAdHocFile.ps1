...
function DropProdUsers{
 Param([string]$destinationInstance, [string]$destinationDbName)

 try
  {
 
  $query=@("
DECLARE @username VARCHAR(64),@DeleteThem INT = 0
...
DEALLOCATE c1;
  ...
  #Set the connection string
    $sqlConn = new-object System.Data.SqlClient.sqlConnection "server=$destinationInstance;database=$destinationDbName;Integrated Security=true"
    ...
    $sqlConn.Open()
        ...
        Write-Host "Prod Users droppped" -ForegroundColor Green
  }
  catch
  {
   CheckForErrors
...
#changed this bit from the original 
 ...
 RestoreDatabase $destinationInstance $destinationDbName $restoreFile "Database"
    ...
    Write-Host
    Write-Host "============================================================="
    Write-Host " 5a: Drop Users from Prod"
    Write-Host "============================================================="

   DropProdUsers $destinationInstance $destinationDbName
